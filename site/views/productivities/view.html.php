...
class ChronosViewProductivities extends JView
{
	...
	*/
	protected function displayDefault($tpl = null)
	{
		...
		//Source
		$this->filters['source'] = new stdClass();
		$this->filters['source']->list = $lists['enum']['productivities.source'];
		array_unshift($this->filters['source']->list, array("value"=>"", "text" => JText::_("CHRONOS_FILTER_NULL_SOURCE")));
		$this->filters['source']->value = $model->getState("filter.source");
		...
		// New
		if ($model->canCreate())
			JToolBarHelper::addNew('productivity.add', "CHRONOS_JTOOLBAR_NEW");

		// Edit
		if ($model->canEdit())
			JToolBarHelper::editList('productivity.edit', "CHRONOS_JTOOLBAR_EDIT");

		// Delete
		if ($model->canDelete())
			JToolBarHelper::deleteList(JText::_('CHRONOS_JTOOLBAR_ARE_YOU_SURE_TO_DELETE'), 'productivity.delete', "CHRONOS_JTOOLBAR_DELETE");
