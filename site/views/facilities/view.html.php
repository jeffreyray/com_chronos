...
class ChronosViewFacilities extends JView
{
	...
	protected function displayAjax($tpl = null)
	{
		...
		switch($render)
		{
			case 'groupby1':
				...
				$ajaxNamespace = "chronos.employees.ajax.groupby1";
				$wrapper = "_ajax_employees_$render";
				$event = 'jQuery("#jform_referrer").val(""); if(this.value != ""){
			...
			case 'groupby2':
				...
				$ajaxNamespace = "chronos.employees.ajax.groupby2";
				$wrapper = "_ajax_employees_$render";
				$event = 'jQuery("#jform_employee").val(""); if(this.value != ""){
						jQuery("#' . $wrapper . '").jdomAjax({namespace:"' . $ajaxNamespace . '", vars:{"filter_facility":this.value}})
						}else{jQuery("#' . $wrapper . '").innerHTML = "";}';
				echo "<div class='ajaxchain-filter ajaxchain-filter-hz'>";
				echo JDom::_('html.form.input.select', array(
	...
	*/
	protected function displayDefault($tpl = null)
	{
		...
		// New
		if ($model->canCreate())
			JToolBarHelper::addNew('facility.add', "CHRONOS_JTOOLBAR_NEW");

		// Edit
		if ($model->canEdit())
			JToolBarHelper::editList('facility.edit', "CHRONOS_JTOOLBAR_EDIT");

		// Delete
		if ($model->canDelete())
			JToolBarHelper::deleteList(JText::_('CHRONOS_JTOOLBAR_ARE_YOU_SURE_TO_DELETE'), 'facility.delete', "CHRONOS_JTOOLBAR_DELETE");
