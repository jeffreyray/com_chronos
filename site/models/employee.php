...
class ChronosModelEmployee extends ChronosModelItem
{
	...
	*/
	...
	{
		$jinput = new JInput;
		return $jinput->get('layout', 'view', 'STRING');
	}
	...
	*/
	public function populateState($ordering = null, $direction = null)
	{
		...
		if ($filter_position = $app->getUserState($this->context.'.filter.position'))
			$this->setState('filter.position', $filter_position, null, 'int');

		if ($search_search_name = $app->getUserState($this->context.'.search.search_name'))
			$this->setState('search.search_name', $search_search_name, null, 'varchar');

		if ($filter_active = $app->getUserState($this->context.'.filter.active'))
			$this->setState('filter.active', $filter_active, null, 'cmd');
	...
	protected function prepareQuery(&$query, $pk)
	{
		...
		switch($this->getState('context'))
		{
			case 'employee.view':
				...
				$this->addSelect('_referral_source_.label AS `_referral_source_label`');
				$this->addSelect('_referrer_.first_name AS `_referrer_first_name`');
				$this->addSelect('_termination_reason_.label AS `_termination_reason_label`');
				...
				$this->addJoin('`chr_referralsources` AS _referral_source_ ON _referral_source_.id = a.referral_source', 'LEFT');
				$this->addJoin('`chr_employees` AS _referrer_ ON _referrer_.id = a.referrer', 'LEFT');
				$this->addJoin('`chr_terminationreasons` AS _termination_reason_ ON _termination_reason_.id = a.termination_reason', 'LEFT');
			...
			case 'employee.edit':
				...
				$this->addSelect('_position_.label AS `_position_label`');
				$this->addSelect('_referral_source_.label AS `_referral_source_label`');
				$this->addSelect('_referrer_.number AS `_referrer_number`');
				$this->addSelect('_referrer_.facility AS `_referrer_facility`');
				$this->addSelect('_referrer_facility_.label AS `_referrer_facility_label`');
				$this->addSelect('_termination_reason_.label AS `_termination_reason_label`');
				$this->addSelect('_checked_out_.name AS `_checked_out_name`');
				...
				$this->addJoin('`chr_referralsources` AS _referral_source_ ON _referral_source_.id = a.referral_source', 'LEFT');
				$this->addJoin('`chr_employees` AS _referrer_ ON _referrer_.id = a.referrer', 'LEFT');
				$this->addJoin('`chr_facilities` AS _referrer_facility_ ON _referrer_facility_.id = _referrer_.facility', 'LEFT');
				$this->addJoin('`chr_terminationreasons` AS _termination_reason_ ON _termination_reason_.id = a.termination_reason', 'LEFT');
				$this->addJoin('`#__users` AS _checked_out_ ON _checked_out_.id = a.checked_out', 'LEFT');
