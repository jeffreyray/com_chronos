...
* @subpackage	Classes
*/
class ChronosModelWorkedshift extends ChronosModelItem
{
	/**
	* View list alias
	*
	...
	* @var string
	*/
	protected $view_list = 'workedshifts';
	...
	*/
	...
	/**
	* Method to delete a workedshift
	*
	* @access	public
	...
	*/
	...
	{
		$jinput = new JInput;
		return $jinput->get('layout', 'workedshift', 'STRING');
	}
	...
	* @since	1.6
	*/
	public function getTable($type = 'workedshift', $prefix = 'ChronosTable', $config = array())
	{
	...
	*/
	public function hit()
	{
		return parent::hit(array('workedshift', 'view'));
	}
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_chronos.edit.workedshift.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('workedshift.id') == 0)
			{
				$jinput = new JInput;
				...
				$data->time_in = null;
				$data->time_out = null;
				$data->break_length = null;
				$data->scheduled_shift = $jinput->get('filter_scheduled_shift', $this->getState('filter.scheduled_shift'), 'INT');
				$data->comment = null;
	...
	* @param	integer	$pk	The primary id key of the workedshift
	* @return	void
	*/
	protected function prepareQuery(&$query, $pk)
	{
		...
		{
			case 'workedshift.workedshift':
				break;

			case 'workedshift.view':
				...
				$this->addSelect('_scheduled_shift_.start AS `_scheduled_shift_start`');
				...
				$this->addJoin('`chr_scheduledshifts` AS _scheduled_shift_ ON _scheduled_shift_.id = a.scheduled_shift', 'LEFT');
