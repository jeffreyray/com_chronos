...
window.addEvent('domready', function()
{
	...
	document.formvalidator.validate = function(el, prepare, args)
	{
		...
		if (type == 'file')
		{
			var current = $('__' + el.getProperty('id'));
		}
	...
	document.formvalidator.validatorTest = function(handler, call, el, regex, args)
	{
		...
		args.regex = regex;
		args.element = el;
		args.name = el.getProperty('name');
		args.id = el.getProperty('id');
			...
			case 'fieldset':
				...
				el.getElements('input').each(function(elem)
				{
					if (elem.getProperty('checked'))
						inputs.push(elem.getProperty('value'));
				});
				value = inputs.join(",");
				break;
		}
			...
			switch(tag + ':' + type)
			{
				...
				case 'input:checkbox':
					...
					el.getElements('input').each(function(inp)
					{
						...
						if ((inp.getProperty('name') == name)
							&& inp.getProperty('checked'))
						{
							inputs.push(inp);
						}
					});
					...
					if (inputs.length == 0)
						...
						value = inputs[0].getProperty('value');
	...
	document.formvalidator.getText = function(el, name)
	{
		if (requiredMooToolsVersion('1.3'))
			return el.getProperty('text').trim();
		else
			return el.getText().trim();

	};

	document.formvalidator.getTag = function(el)
	{
		if (requiredMooToolsVersion('1.3'))
			var tag = el.get('tag');
		else
