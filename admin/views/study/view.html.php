...
class ChronosViewStudy extends JView
{
	...
	*/
	public function display($tpl = null)
	{
		...
		if (!in_array($layout, array('study')))
			return;
	...
	/**
	* Execute and display a template : Study
	*
	* @access	protected
	* @param	string	$tpl	The name of the template file to parse; automatically searches through the template paths.
	*
	...
	* @since	11.1
	*/
	protected function displayStudy($tpl = null)
	{
		$document	= &JFactory::getDocument();
		$document->title = $document->titlePrefix . JText::_("CHRONOS_LAYOUT_STUDY") . $document->titleSuffix;

		// Initialiase variables.
		$this->model	= $model	= $this->getModel();
		$this->state	= $state	= $this->get('State');
		$state->set('context', 'study.study');
		...
		JToolBarHelper::title(JText::_('CHRONOS_LAYOUT_STUDY'), 'chronos_studies');
