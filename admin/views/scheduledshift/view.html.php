...
class ChronosViewScheduledshift extends JView
{
	...
	*/
	public function display($tpl = null)
	{
		...
		if (!in_array($layout, array('scheduledshift')))
			return;
	...
	/**
	* Execute and display a template : Scheduled Shift
	*
	* @access	protected
	* @param	string	$tpl	The name of the template file to parse; automatically searches through the template paths.
	*
	...
	* @since	11.1
	*/
	protected function displayScheduledshift($tpl = null)
	{
		$document	= &JFactory::getDocument();
		$document->title = $document->titlePrefix . JText::_("CHRONOS_LAYOUT_SCHEDULED_SHIFT") . $document->titleSuffix;

		// Initialiase variables.
		$this->model	= $model	= $this->getModel();
		$this->state	= $state	= $this->get('State');
		$state->set('context', 'scheduledshift.scheduledshift');
		...
		JToolBarHelper::title(JText::_('CHRONOS_LAYOUT_SCHEDULED_SHIFT'), 'chronos_scheduledshifts');

		// Save
		if (($isNew && $model->canCreate()) || (!$isNew && $item->params->get('access-edit')))
			JToolBarHelper::save('scheduledshift.save', "JTOOLBAR_SAVE");

		// Apply
		if ($item->params->get('access-edit'))
			JToolBarHelper::apply('scheduledshift.apply', "JTOOLBAR_APPLY");

		// Cancel
		JToolBarHelper::cancel('scheduledshift.cancel', "JTOOLBAR_CANCEL");
