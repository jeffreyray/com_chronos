...
class ChronosViewUmbrella extends JView
{
	...
	*/
	public function display($tpl = null)
	{
		...
		if (!in_array($layout, array('umbrella')))
			return;
	...
	/**
	* Execute and display a template : Umbrella
	*
	* @access	protected
	* @param	string	$tpl	The name of the template file to parse; automatically searches through the template paths.
	*
	...
	* @since	11.1
	*/
	protected function displayUmbrella($tpl = null)
	{
		$document	= &JFactory::getDocument();
		$document->title = $document->titlePrefix . JText::_("CHRONOS_LAYOUT_UMBRELLA") . $document->titleSuffix;

		// Initialiase variables.
		$this->model	= $model	= $this->getModel();
		$this->state	= $state	= $this->get('State');
		$state->set('context', 'umbrella.umbrella');
		...
		JToolBarHelper::title(JText::_('CHRONOS_LAYOUT_UMBRELLA'), 'chronos_umbrellas');

		// Save
		if (($isNew && $model->canCreate()) || (!$isNew && $item->params->get('access-edit')))
			JToolBarHelper::save('umbrella.save', "JTOOLBAR_SAVE");

		// Apply
		if ($item->params->get('access-edit'))
			JToolBarHelper::apply('umbrella.apply', "JTOOLBAR_APPLY");

		// Cancel
		JToolBarHelper::cancel('umbrella.cancel', "JTOOLBAR_CANCEL");
