...
* @subpackage	Classes
*/
class ChronosModelWagereason extends ChronosModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function delete(&$pks = array())
	{
		...
		//Integrity : Reset fk : reason in wage
		$model = JModel::getInstance('wage', 'ChronosModel');
		if (!$model->integrityReset('reason', $pks))
		{
	...
	*/
	...
	{
		$jinput = new JInput;
		return $jinput->get('layout', 'wagereason', 'STRING');
	}
	...
	* @since	1.6
	*/
	public function getTable($type = 'wagereason', $prefix = 'ChronosTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_chronos.edit.wagereason.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('wagereason.id') == 0)
			{
				$jinput = new JInput;
	...
	* @param	integer	$pk	The primary id key of the wagereason
	* @return	void
	*/
	protected function prepareQuery(&$query, $pk)
	{
		...
		//FROM : Main table
		$query->from('chr_wagereasons AS a');
