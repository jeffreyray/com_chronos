...
* @subpackage	Classes
*/
class ChronosModelUmbrella extends ChronosModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function delete(&$pks = array())
	{
		...
		//Integrity : Reset fk : umbrella in study
		$model = JModel::getInstance('study', 'ChronosModel');
		if (!$model->integrityReset('umbrella', $pks))
		{
	...
	*/
	...
	{
		$jinput = new JInput;
		return $jinput->get('layout', 'umbrella', 'STRING');
	}
	...
	* @since	1.6
	*/
	public function getTable($type = 'umbrella', $prefix = 'ChronosTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_chronos.edit.umbrella.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('umbrella.id') == 0)
			{
				$jinput = new JInput;
	...
	* @param	integer	$pk	The primary id key of the umbrella
	* @return	void
	*/
