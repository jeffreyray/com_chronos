...
@define("CHRONOS_UPLOAD_EXTENSIONS_JOINED", "");

class ChronosFile extends JFile
{
	...
	function fileExtension($file)
	{
		...
		foreach(explode(",", CHRONOS_UPLOAD_EXTENSIONS_JOINED) as $joinedExt)
		{
	...
	function downloadUrl($path, $dirPattern, $options = array())
	{
		...
		$url = JURI::base(true) . "/index.php?option=com_chronos&task=file"
			.	"&path=" . $path
			. 	"&action=download";
	...
	function getMarkers()
	{
		...
		return array(
			'DIR_EMPLOYEES_IMAGE' => $config->get("upload_dir_employees_image", "[COM_SITE]" .DS. "files" .DS. "employees_image") .DS,
			'DIR__TRASH' => $config->get("trash_dir", JPATH_ADMIN_CHRONOS .DS. "images" . DS . "trash") .DS,

			'COM_ADMIN' => JPATH_ADMIN_CHRONOS,
			'ADMIN' => JPATH_ADMINISTRATOR,
			'COM_SITE' => JPATH_SITE_CHRONOS,
	...
	function returnFile($mode = null)
	{
		...
		)
		{
			...
			require_once(JPATH_ADMIN_CHRONOS .DS. 'classes' .DS. 'images.php');
			$thumb = new ChronosImages($filePath, $mime);
		...
		else if (!JFile::exists($filePath))
		{
			...
			$msg = JText::sprintf( "CHRONOS_UPLOAD_FILE_NOT_FOUND", $path);
