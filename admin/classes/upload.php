...
@define("CHRONOS_UPLOAD_RANDOM_CHARS", "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789");
@define("CHRONOS_UPLOAD_DEFAULT_MAX_SIZE", 2097152);  //2Mb


//TODO
@define("CHRONOS_UPLOAD_CHMOD_FOLDER", 0744);
@define("CHRONOS_UPLOAD_CHMOD_FILE", 0644);


class ChronosUpload extends ChronosFile
{
	...
	function getMaxSize($string = false, $maxSizeCustom = null)
	{
		...
		if (!$maxSize)
			$maxSize = CHRONOS_UPLOAD_DEFAULT_MAX_SIZE;

		if ($maxSizeCustom)
			$maxSize = min($maxSize, $maxSizeCustom);

		if ($string)
			$maxSize = JText::sprintf("CHRONOS_UPLOAD_MAX_B", self::bytesToString($maxSize));
	...
	function setUploadFolder($uploadFolder)
	{
		...
		@chmod($uploadPath, CHRONOS_UPLOAD_CHMOD_FOLDER);
		if(!is_writable($uploadPath))
		{
			$app->enqueueMessage(JText::sprintf( "CHRONOS_UPLOAD_PLEASE_MAKE_SURE_THE_FOLDER_IS_WRITABLE",$uploadPath), 'notice');
			return false;
		}
	...
	function uploadFile($uploadFile, $options = array())
	{
		...
		if(empty($uploadFile['name'])){
			$app->enqueueMessage(JText::_("CHRONOS_UPLOAD_PLEASE_BROWSE_A_FILE"),'notice');
			return false;
		}
...
//CHECK EXTENSION
		...
		{
			$app->enqueueMessage(JText::sprintf( "CHRONOS_UPLOAD_THIS_FILE_EXTENSION_IS_NOT_ACCEPTED_THE_ACCEPTED_FILES_ARE",
												$file->extension,
...
//CHECK MIME HEADER
		...
		{
			$app->enqueueMessage(JText::sprintf( "CHRONOS_UPLOAD_MIME_TYPE_NOT_VALID_ALLOWED_MIMES_ARE",
												$this->file->mime,
...
//CHECK SIZE
		...
		{
			$app->enqueueMessage(JText::sprintf( "CHRONOS_UPLOAD_TOO_BIG_FILE_BYTES_MAX_ALLOWED_SIZE_BYTES",
...
//CHECK PHP INJECTION
		...
		{
			$app->enqueueMessage(JText::_( "CHRONOS_UPLOAD_THE_FILE_CONTAINS_ERRORS"), 'error');
			return false;
		}
...
//CHECK FILE PRESENCE
		...
		{
			$app->enqueueMessage(JText::sprintf( "CHRONOS_UPLOAD_THIS_FILE_ALREADY_EXIST",$file->filename), 'notice');
			return false;
		}
...
//PROCESS UPLOAD
		...
		{
			if ($app->isSite())
				$msg = JText::sprintf( "CHRONOS_UPLOAD_COULD_NOT_UPLOAD_THE_FILE", $file->filename);	// Don't show the complete directory in front-end
			else if ($app->isAdmin())
				$msg = JText::sprintf( "CHRONOS_UPLOAD_COULD_NOT_UPLOAD_THE_FILE_TO",$file->tmp,$this->uploadFolder . $file->filename);
	...
	function process()
	{
		...
		@chmod($fileDest, CHRONOS_UPLOAD_CHMOD_FILE);
	...
	{
		$lenChars = strlen(CHRONOS_UPLOAD_RANDOM_CHARS);
		$random = "";
		...
		{
			$pos = rand(0, $lenChars);
			$random .= substr(CHRONOS_UPLOAD_RANDOM_CHARS, $pos, 1);
		}
