...
@define("CHRONOS_IMAGES_MAX_WIDTH", 1000);
@define("CHRONOS_IMAGES_MAX_HEIGHT", 1000);
@define("CHRONOS_IMAGES_ALOWED_SIZES", '');
@define("CHRONOS_IMAGES_PHYSICAL_THUMB", true);		//Create an hidden thumb file with parameters
@define("CHRONOS_IMAGES_FALLBACK_NAME", ".notfound.png");
@define("CHRONOS_IMAGES_FALLBACK_ROOT", JPATH_ADMIN_CHRONOS .DS. "images");

require_once(JPATH_ADMIN_CHRONOS .DS. "classes" .DS. "file.php");


class ChronosImages  extends ChronosFile
{
	...
	function searchFile(&$create)
	{
		...
		$fallback = $this->dir .DS. CHRONOS_IMAGES_FALLBACK_NAME;
		...
		$fallback = CHRONOS_IMAGES_FALLBACK_ROOT .DS. CHRONOS_IMAGES_FALLBACK_NAME;
		if (JFile::exists($fallback))
		{
			$create = 'fly';
			return $fallback;
		...
		switch($create)
		{
			case 'thumb':
				...
				else
				{
					...
					if (($size == "0x0") || in_array($size, explode(",", CHRONOS_IMAGES_ALOWED_SIZES)))
					{
						$image = $this->load();
...
*/
    ...
    function writeThumb($image, $name=null)
    {
    	...
    	if (!CHRONOS_IMAGES_PHYSICAL_THUMB)
	...
	{
		$this->width = min($width, CHRONOS_IMAGES_MAX_WIDTH);
	}

    function height($height='') {
        $this->height = min($height, CHRONOS_IMAGES_MAX_HEIGHT);
    }
